...
object Day9:
  def solve(input: String): Any =
    ...
    (for
      coords <- signals.values.toVector
      c1 <- coords
      c2 <- coords
      if c1 != c2
      antinode = Coord(c1.x - (c2.x - c1.x), c1.y - (c2.y - c1.y))
      if 0 <= antinode.x && 0 <= antinode.y && antinode.x <= grid.keys.map(_.x).max && antinode.y <= grid.keys.map(_.y).max
    yield antinode).size
