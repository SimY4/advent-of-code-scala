  ...
  def solve(input: String): Int = {
    ...
    @tailrec def loop(coord: Coord, acc: Int = 0): Int =
      area.lift((coord.y + 1L).toInt) match {
        case None => acc
        case Some(axis) => loop(Coord(x = coord.x + 3L, y = coord.y + 1L), if (axis((coord.x + 3L).toInt) == '#') acc + 1 else acc)
      }
  ...
  def solve2(input: String): Long = {
    ...
    @tailrec def loop(coord: Coord, slope: Coord, acc: Int = 0): Long =
      area.lift((coord.y + slope.y).toInt) match {
        ...
        case Some(axis) => loop(Coord(x = coord.x + slope.x, y = coord.y + slope.y), slope, if (axis((coord.x + slope.x).toInt) == '#') acc + 1 else acc)
      }
