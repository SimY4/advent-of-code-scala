...
object Day11 {
    ...
    @tailrec def loop(layout: Array[Array[Char]]): Int = {
      val newLayout = layout.zipWithIndex.map { (col, y) => 
        col.zipWithIndex.map { 
          case ('L', x) if !Coord(x, y).neighbours().flatMap(n => layout.lift(n.y.toInt).flatMap(_.lift(n.x.toInt))).exists(_ == '#') => '#'
          case ('#', x) if 4 <= Coord(x, y).neighbours().flatMap(n => layout.lift(n.y.toInt).flatMap(_.lift(n.x.toInt))).count(_ == '#') => 'L'
          case (c, _) => c
        }
      }
  ...
  val input = """LLLLLLLLL.LLLLLLL.LLLLLLLLLL.LLLLL.LLLLLL.LLLLLLLL.L.LLLLLL..L.LLLLL.LLLLLLLLLL.LLLLLLLLLLL
