...
object Day2:
  @tailrec private def runProgram(pointer: Int, opCodes: Vector[Int]): Int = opCodes(pointer) match
    case 1 =>
      val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
      runProgram(pointer + 4, opCodes.updated(z, opCodes(x) + opCodes(y)))
    case 2 =>
      val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
      runProgram(pointer + 4, opCodes.updated(z, opCodes(x) * opCodes(y)))
    case 99 => opCodes(0)
  ...
  def solve2(input: String): Option[Int] =
    ...
    yield noun * 100 + verb).headOption
