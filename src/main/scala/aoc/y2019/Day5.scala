...
object Day5:
  ...
  def runProgram(opCodes: Vector[Int], inputs: LazyList[Int]): LazyList[Int] =
    LazyList
      .iterate(ProgramState(0, opCodes, inputs, None)):
        case ProgramState(pointer, opCodes, inputs, _) =>
          opCodes(pointer) match
            case 1 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, opCodes(x) + opCodes(y)), inputs, None)
            case 101 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, x + opCodes(y)), inputs, None)
            case 1001 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, opCodes(x) + y), inputs, None)
            case 1101 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, x + y), inputs, None)
            case 2 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, opCodes(x) * opCodes(y)), inputs, None)
            case 102 =>
            ...
            case 105 =>
              val (x, y) = (opCodes(pointer + 1), opCodes(pointer + 2))
              ProgramState(if x != 0 then opCodes(y) else pointer + 3, opCodes, inputs, None)
            case 1005 =>
              val (x, y) = (opCodes(pointer + 1), opCodes(pointer + 2))
              ProgramState(if opCodes(x) != 0 then y else pointer + 3, opCodes, inputs, None)
            case 1105 =>
              val (x, y) = (opCodes(pointer + 1), opCodes(pointer + 2))
              ProgramState(if x != 0 then y else pointer + 3, opCodes, inputs, None)
            case 6 =>
              val (x, y) = (opCodes(pointer + 1), opCodes(pointer + 2))
              ProgramState(if opCodes(x) == 0 then opCodes(y) else pointer + 3, opCodes, inputs, None)
            case 106 =>
            ...
            case 1108 =>
              val (x, y, z) = (opCodes(pointer + 1), opCodes(pointer + 2), opCodes(pointer + 3))
              ProgramState(pointer + 4, opCodes.updated(z, if x == y then 1 else 0), inputs, None)
            case _ => ProgramState(-1, Vector.empty, inputs, None)
      .takeWhile(_.pointer >= 0)
      .collect { case ProgramState(_, _, _, Some(output)) => output }

  def solve(input: String): String =
    runProgram(input.split(",").map(_.toInt).toVector, LazyList(1)).mkString

  def solve2(input: String): String =
    runProgram(input.split(",").map(_.toInt).toVector, LazyList(5)).mkString
