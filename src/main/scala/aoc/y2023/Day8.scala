...
object Day8:
  private def parse(input: String): (Vector[Direction], Map[String, (String, String)]) =
    input.split("\\n\\n").toList match
      case directions :: network :: Nil =>
        (
          directions
            .map:
            ...
            .toVector,
          network.linesIterator
            .map:
              case s"$node = ($l, $r)" => node -> (l, r)
            .toMap
        )

  def solve(input: String): Int =
    val (directions, network)              = parse(input)
    def directionsSeq: LazyList[Direction] = LazyList.from(directions) #::: directionsSeq
