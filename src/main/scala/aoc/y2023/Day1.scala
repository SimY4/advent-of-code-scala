...
object Day1:
  def solve(input: String): Int =
    input.linesIterator
      .flatMap: line =>
        for
          fst <- line.find(_.isDigit)
          lst <- line.findLast(_.isDigit)
        yield fst.asDigit * 10 + lst.asDigit
      .sum

  extension (s: String)
    private def digits: Vector[Int] =
      "(?=(one|two|three|four|five|six|seven|eight|nine|\\d))".r
        .findAllMatchIn(s)
        .map: m =>
          m.group(1) match
            case "one"   => 1
