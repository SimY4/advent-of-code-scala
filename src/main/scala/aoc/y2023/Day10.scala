...
object Day10:
  private val moves = Map(
    Direction.Down  -> Set('7', '|', 'F'),
    Direction.Up    -> Set('J', '|', 'L'),
    Direction.Right -> Set('7', '-', 'J'),
    Direction.Left  -> Set('L', '-', 'F')
  )

  @tailrec private def makeLoop(
    maze: Vector[String],
    node: Option[Coord],
    ...
    node match
      ...
      case Some(node) =>
        val next =
          maze(node)
            .match
              case 'S' => if path.isEmpty then Direction.hvOnly else Nil
              case '|' => List(Direction.Up, Direction.Down)
              case '-' => List(Direction.Right, Direction.Left)
              case 'L' => List(Direction.Down, Direction.Right)
              case 'J' => List(Direction.Down, Direction.Left)
              case 'F' => List(Direction.Up, Direction.Right)
              case '7' => List(Direction.Up, Direction.Left)
            .map(d => d -> (node + d.direction))
            .collectFirst:
              case (d, n) if maze.get(n).exists(moves(d)) && !path(n) => n
  ...
  def solve2(input: String): Int =
    ...
    val doubleLoop = loop
      ...
      .union(
        loop.toList.pairs
          .map((f, s) => Coord(f.x + s.x, f.y + s.y))
          .toSet
      )
    val (maxx, maxy) = (
      if maze.head.length() % 2 == 0 then maze.head.length() * 2 - 1 else maze.head.length() * 2 - 2,
      if maze.size          % 2 == 0 then maze.size * 2 - 1 else maze.size * 2 - 2
    )

    @tailrec def countOuter(nodes: Set[Coord], path: Set[Coord] = Set.empty): Set[Coord] =
      if nodes.isEmpty then path
      else
        val next = nodes
          .flatMap: node =>
            node
              .neighbours()
              .filter: n =>
                0 <= n.x && n.x <= maxx && 0 <= n.y && n.y <= maxy && !path(n) && !doubleLoop(
                  n
                )
        countOuter(next, path ++ nodes)

    val outer = countOuter(
      ((0 to maxx).flatMap(sz => List(Coord(sz, 0), Coord(sz, maxy))) ++
        (0 to maxy).flatMap(sz => List(Coord(0, sz), Coord(maxx, sz)))).filterNot(doubleLoop).toSet
    ).filter(c => c.x % 2 + c.y % 2 == 0)

    maze.map(_.length()).sum - outer.size - loop.size
  ...
  val input =
    """F-|-J7F7F7F-7F7F--7-L|-L7777.FFL-|.|.FF-FF7FFFFF7--F-J7.|-FF77|-F|777-F7.F7FF|-J7|-7.FFJ77FL.7-|-|7.|.77|-F-|-F|-FL.F77.FF-77|7.7--7--7FJ--7
      ...
      |LJJ..LL-JJ-L---7J-F77LL.LFJJLL-J-7JJ.L-LL7-L7.LLJL|JJF7-J-JLLJLLJ-7LJJ..LLJ-LJJ.LFL-FJJL|.FLLF-FL|LL.J.FL|-JJ.JJL--JLL--JJ.|J-L|J--J.L7..JJJ""".stripMargin
