...
object Day2:
  final private case class Game(id: Int, sets: List[(Int, Int, Int)]):
    ...
    def power: Int =
      val (r, g, b) = sets.foldLeft((0, 0, 0)):
        case ((ar, ag, ab), (r, g, b)) => (ar max r, ag max g, ab max b)
      r * g * b
          ...
          sets
            ...
            .map: set =>
              set
                .split(',')
                .foldLeft((0, 0, 0)):
                  case ((ar, ag, ab), s" $r red")   => (ar + r.toInt, ag, ab)
                  case ((ar, ag, ab), s" $g green") => (ar, ag + g.toInt, ab)
                  case ((ar, ag, ab), s" $b blue")  => (ar, ag, ab + b.toInt)
            .toList
        )

  def solve(input: String, r: Int = 12, g: Int = 13, b: Int = 14): Int =
    (for
      line <- input.linesIterator
      game = parseLine(line)
      if game.possible(r, g, b)
    yield game.id).sum
  ...
  val input =
    """Game 1: 4 red, 5 blue, 4 green; 7 red, 8 blue, 2 green; 9 blue, 6 red; 1 green, 3 red, 7 blue; 3 green, 7 red
      ...
      |Game 100: 4 blue, 1 green; 13 red, 2 blue; 16 red; 15 red, 2 blue; 9 red, 1 green, 1 blue; 7 red, 4 blue""".stripMargin
