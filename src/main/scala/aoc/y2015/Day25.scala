...
object Day25 {
  private final case class Cell(coord: Coord, incRow: Long)

  def solve(input: Coord): Option[Long] = 
    LazyList.iterate(Cell(Coord(1L, 1L), 1L) -> 20151125L) { case (Cell(Coord(row, col), incRow), value) =>
      val nextValue = value * 252533L % 33554393L
      if (incRow == col) Cell(Coord(incRow + 1L, 1L), incRow + 1L) -> nextValue
